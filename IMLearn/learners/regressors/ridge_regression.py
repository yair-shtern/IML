...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.include_intercept_:
            X = np.concatenate((np.ones((np.size(X, axis=0), 1)), X), axis=1)
            id_lam = self.lam_ * np.identity(X.shape[1])
            id_lam[0, 0] = 0
        else:
            id_lam = self.lam_ * np.identity(X.shape[1])
        self.coefs_ = pinv(X.T @ X + id_lam) @ (X.T @ y)
    ...
    def _loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        return np.sum((y - self._predict(X)) ** 2) / y.shape[0]
