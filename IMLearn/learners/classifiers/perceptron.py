...
class Perceptron(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        self.fitted_ = True
        iterations = 0
        temp = np.sign(X @ self.coefs_) - y
        while np.any(temp) and iterations < self.max_iter_:
            bad_index = np.nonzero(temp)[0][0]
            self.coefs_ += X[bad_index] * y[bad_index]
            temp = np.sign(X @ self.coefs_) - y
            iterations += 1
            self.callback_(self, X, iterations)
