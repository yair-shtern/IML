...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.penalty_ == "none":
            self.coefs_ = self.solver_.fit(f=LogisticModule(self.coefs_), X=X, y=y).T
        else:
            module = RegularizedModule(fidelity_module=LogisticModule(),
            ...
            self.coefs_ = self.solver_.fit(f=module, X=X, y=y).T
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        array = np.array(self.predict_proba(X) >= self.alpha_)
