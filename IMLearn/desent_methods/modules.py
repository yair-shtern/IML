...
class L2(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        """
        return np.sum(self.weights_ * self.weights_)
...
class LogisticModule(BaseModule):
    ...
    Represents the function: f(w) = - (1/m) sum_i^m[y*<x_i,w> - log(1+exp(<x_i,w>))]
        ...
        """
        m = X.shape[0]
        log_part = np.log(1 + np.exp(X @ self.weights_))
        val_for_sum = y * (X @ self.weights_) - log_part
        sum_val = np.sum(val_for_sum, axis=0)
        return - (1 / m) * sum_val
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        m = X.shape[0]
        denominator = np.exp(np.dot(X, self.weights_)) + 1
        numerator = np.exp(np.dot(X, self.weights_))
        return (-1 / m) * ((y - (numerator / denominator)) @ X)
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        regularization_compute = self.regularization_module_.compute_output(**kwargs)
        fidelity_compute = self.fidelity_module_.compute_output(**kwargs)
        return fidelity_compute + self.lam_ * regularization_compute
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        regularization_compute = self.regularization_module_.compute_jacobian(**kwargs)
        if self.include_intercept_:
            mat = np.asmatrix(regularization_compute)
            regularization_compute = np.insert(mat, 0, 0, axis=1)

        fidelity_compute = self.fidelity_module_.compute_jacobian(**kwargs)
        return fidelity_compute + self.lam_ * regularization_compute
