...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        for num_iterations in range(self.max_iter_):
            eta = self.learning_rate_.lr_step(t=num_iterations)
            f.weights_ = path[num_iterations]
            direction = - f.compute_jacobian(X=X, y=y)
            x_t = path[num_iterations] + eta * direction
            path.append(x_t)
            f.weights_ = x_t
            vec = path[num_iterations] - path[num_iterations + 1]
            delta = np.linalg.norm(vec)
            curr_loss = f.compute_output(X=X, y=y)

            if curr_loss < min_loss:
                min_loss = curr_loss
                best_weight = path[num_iterations + 1]

            self.callback_(output=self, weights=f.weights_, val=curr_loss,
                           grad=f.compute_jacobian(X=X, y=y), t=num_iterations, eta=eta, delta=delta)
            if delta < self.tol_:
                break

        return_value = {OUTPUT_VECTOR_TYPE[0]: path[-1],
                        OUTPUT_VECTOR_TYPE[1]: best_weight,
                        OUTPUT_VECTOR_TYPE[2]: np.mean(np.array(path))}
        return return_value[self.out_type_]
